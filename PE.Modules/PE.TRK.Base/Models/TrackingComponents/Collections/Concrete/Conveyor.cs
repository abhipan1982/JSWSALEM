...
{
  public class Conveyor : TrackingQueueBase
  {
    #region ctor

    public Conveyor(int queueSize, int assetCode, TrackingEventType trackingEventType,
    ...
    private TrackingQueueElementBase DischargeMaterial()
    {
      lock (LockObject)
      {
        ...
        if (position == null)
        {
          throw new Exception("For DischargeElement operation in Conveyor Area should be at least one material");
        }
