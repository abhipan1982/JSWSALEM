...
{
  public class Transport : TrackingQueueBase
  {
    #region ctor

    public Transport(int queueSize, int assetCode, TrackingEventType trackingEventType,
    ...
    private TrackingQueueElementBase DischargeMaterial()
    {
      lock (LockObject)
      {
        ...
        if (position == null)
        {
          throw new Exception("For DischargeElement operation in Transport Area should be at least one material");
        }
