...
CREATE           VIEW [hmi].[V_WorkOrderSummary] AS
...
WITH Materials
     ...
     AS (SELECT FKWorkOrderId, 
		 ...
		 INNER JOIN TRKRawMaterials RM ON P.ProductId = RM.FKProductId AND RM.EnumTypeOfScrap != 2
         GROUP BY FKWorkOrderId),
     RawMaterials
     AS (SELECT M.FKWorkOrderId, 
				...
				SUM(CASE
                        WHEN EnumRejectLocation = 3
                        THEN 1
                        ELSE 0
                    END) AS MaterialsRejectedOnMill, 
                SUM(CASE
				...
				SUM(CASE
                        WHEN RM.EnumRejectLocation = 3
                        THEN RM.LastWeight
                        ELSE 0
                    END) AS MaterialsRejectedWeightOnMill,
				SUM(CASE
                        WHEN RM.EnumRawMaterialStatus > 80
                        THEN 1
                        ELSE 0
                    END) AS MaterialsAfterOnCoolingBed 
     ...
     SELECT ISNULL(ROW_NUMBER() OVER(
            ORDER BY WO.WorkOrderCreatedTs DESC), 0) AS OrderSeq, 
            WO.WorkOrderId, 
            WO.WorkOrderName, 
            WO.IsTestOrder, 
            WO.IsBlocked, 
			...
			ISNULL(RM.MaterialsRejectedOnMill, 0) AS MaterialsRejectedOnMill, 
			...
			ISNULL(RM.MaterialsRejectedWeightOnMill, 0) AS MaterialsRejectedWeightOnMill, 
			...
			ISNULL(RM.MaterialsAfterOnCoolingBed, 0) AS MaterialsAfterOnCoolingBed,
			CAST(CASE
				WHEN RM.MaterialsAfterOnCoolingBed = RM.RawMaterialsNumber
				THEN 1
				ELSE 0
			END AS BIT) AS AllRawMaterialsWithStatusHigher80
