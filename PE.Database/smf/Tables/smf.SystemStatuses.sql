...
GO
CREATE   TRIGGER [smf].[TR_SystemStatuses_Update] ON [smf].[SystemStatuses] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @DateTS DATETIME, @Status INT, -- 0-Unknown; 1-Error, 2-Warning, 3-OK
    @Id BIGINT;
    SET @DateTS = GETDATE();
    SELECT @Status = inserted.STATUS, 
           @Id = inserted.SystemStatusId
    FROM INSERTED;
    UPDATE smf.SystemStatuses
      SET 
          LastUpdateTs = @DateTS
    FROM smf.SystemStatuses, inserted
    WHERE smf.SystemStatuses.SystemStatusId = @Id;
