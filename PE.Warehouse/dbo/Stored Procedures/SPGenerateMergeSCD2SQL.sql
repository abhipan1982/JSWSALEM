CREATE   PROCEDURE [dbo].[SPGenerateMergeSCD2SQL] @TableName VARCHAR(50)
AS

/*
	EXEC dbo.SPGenerateMergeSCD2SQL 'DimAsset'
*/

    BEGIN
        SET NOCOUNT ON;
        DECLARE @SQLStat VARCHAR(MAX), @SourceInsertColumns VARCHAR(MAX), @DestInsertColumns VARCHAR(MAX);
        ...
        --DECLARE @TableName VARCHAR(50)= 'DimAsset';
        ...
        CLOSE @ColNames;
        DEALLOCATE @ColNames;
        SET @SQLStat = 'INSERT INTO ' + @DestinationSchema + '.' + @TableName + ' (' + @DestInsertColumns + ') SELECT ' + @DestInsertColumns + ' FROM (
			MERGE ' + @DestinationSchema + '.' + @TableName + ' AS Dst
                USING [' + @LinkedServer + '].' + @SourceDB + '.' + @SourceSchema + '.' + @TableName + ' AS Src
                ON (Dst.' + @KeyColName + ' = Src.' + @KeyColName + ')
            WHEN MATCHED AND Dst.' + @HashColName + ' ! = Src.' + @HashColName + ' AND Dst.' + @TableName + 'IsCurrent = 1 THEN 
				UPDATE SET ' + @TableName + 'IsCurrent = 0
            ...
            WHEN NOT MATCHED BY SOURCE THEN 
				UPDATE SET ' + @TableName + 'IsDeleted = 1 ' + ', ' + @TableName + 'IsCurrent = 0
            OUTPUT $action AS Operation, ' + @SourceInsertColumns + ') AS Merged
       WHERE Merged.Operation = ''UPDATE'' AND ' + @TableName + 'IsDeleted = 0;' + @NewLine;
