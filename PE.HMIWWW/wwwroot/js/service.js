...
signalR.HubConnection.prototype.addToGroup = function (groupName) {
  this.invoke("AddToGroup", groupName);
  console.debug("Registering for group ", groupName);
}
...
$(document).ready(function () {
  var reconnectPolicy = new InfiniteReconnectPolicy();
  ...
  async function start() {
    try {
      ...
      console.error("SignalR Connection problem" + err);
  ...
  SignalrConnection.onreconnecting(error => {
    console.log("SignalR Reconnection" + error);
  });
  SignalrConnection.onreconnected(connectionId => {
    console.log("SignalR Reconnected: " + connectionId);
    RegisterInRefreshHandlersGroups();
  });

  SignalrConnection.on("System2HmiBroadcastShowAlarmPopup", (alarmData) => {
  ...
  SignalrConnection.on("OnConnected", connectionId => {
    console.log("SignalR Connected: " + connectionId);
    SignalrConnection.invoke("SendPageInfo", window.location.pathname);
    VerifyPageVersion();
    RegisterInRefreshHandlersGroups();
  });

  SignalrConnection.on("OnDisconnected", connectionId => {
    console.log("SignalR Disconnected: " + connectionId);
  });
...
var InfiniteReconnectPolicy = (function () {
  function InfiniteReconnectPolicy() { }
  InfiniteReconnectPolicy.prototype._retryDelays = [0, 2000, 10000, 30000];
  InfiniteReconnectPolicy.prototype.nextRetryDelayInMilliseconds = function (retryContext) {
    return retryContext.previousRetryCount < this._retryDelays.length 
      ? this._retryDelays[retryContext.previousRetryCount]
      : this._retryDelays[this._retryDelays.length - 1];
  };
  return InfiniteReconnectPolicy;
}());

// END OF PAGE EVENTS

//HELPER FUNCTIONS
var disablePageRefresCheck = false;
function VerifyPageVersion() {
  if (!$('meta[name=date]').length || disablePageRefresCheck) return;
  $.ajax({
    url: '/Home/GetPublishDate',
    type: "GET",
    success: function (date) {
      var publishDate = $('meta[name=date]').attr('content');
      if (publishDate != date) {
        disablePageRefresCheck = true;
        var pageRefresTimer = true;

        // auto reload page in 15 sec if no option checked
        ...
        PromptMessage(Translations["MESSAGE_PageOutdated"], Translations["MESSAGE_PageOutdatedMessage"], function (confirm) {
          if (confirm) {
            Refresh();
          } else {
            $('meta[name=date]').attr('content', date);
            disablePageRefresCheck = false;
            pageRefresTimer = false;
          }
        });
      }
    }
...
function RegisterInRefreshHandlersGroups() {
  if (!HmiRefreshTarget) return;

  HmiRefreshTarget.listRegisteredKeys().forEach(el => {
    SignalrConnection.addToGroup(el);
  });
}
  ...
  if (e.xhr !== null) {
    if (e.xhr.status === 403) {
      ...
      if (e.xhr.responseJSON)
        ErrorMessage(e.xhr.responseJSON.Data.Errors); //changed for grid
      else
...
function SendRequestWithConfirmation(url, parameters = null, message = '', onSuccessMethod = null) {
  if (!url) return;
...
var textColors = ["fff", "fff", "fff", "000", "fff", "fff", "fff", "fff", "fff", "fff", "fff", "000", "fff", "fff", "fff", "fff"];
...
function GetTextColorModulo16(elementId) {
  return "#" + textColors[elementId % 16];
}
// end of colors
