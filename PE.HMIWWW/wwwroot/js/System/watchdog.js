...
$(document).ready(function () {
    ...
    SignalrConnection.on("System2HmiBroadcastWDogState", (wdogData) => {
...
function BuildWdogTable(data, useTimeout) {
    ...
    try {
        counter++;
        if (counter == counterDisplay.length) counter = 0;
        ...
        for (var ii = 0; ii < data.ProcessStates.length; ii++) {
            ...
            row.find('td').eq(0).text(counterDisplay[counter]+data.ProcessStates[ii].ProgramName);
