...
function groupDataForManyFeatures(chartData) {
  selectedFeatureIds.forEach((id) => {
    ...
    featureMeasurements.forEach((el) => {
      ...
      if (el.Feature.IsSampledFeature) {
        ...
        createSeries(yaxis, el.Material.RawmMaterialName + ' | ' + el.Feature.FeatureName + ' | ' + el.Feature.Unit, samples, yRenderer);
      } else {
        let valueX1 = el.Feature.IsLengthRelated ? 0 : new Date(el.Measurement.FirstMeasurementTs);
        let valueX2 = el.Feature.IsLengthRelated ? el.Measurement.ActualLength : new Date(el.Measurement.LastMeasurementTs);
        samples.push({ valueX: valueX1, valueY: el.Measurement.Avg });
        samples.push({ valueX: valueX2, valueY: el.Measurement.Avg });
        createSeries(yaxis, el.Material.RawmMaterialName + ' | ' + el.Feature.FeatureName + ' | ' + el.Feature.Unit, samples, yRenderer);
      }
...
function groupDataForManyMaterials(chartData) {
  const yRenderer = am5xy.AxisRendererY.new(root, {});
  const yaxis = createAxis(yRenderer);
  chartData.Measurements.forEach((el) => {
    let samples = [];
    if (el.Feature.IsSampledFeature) {
      el.Measurement.Samples.forEach((x) => {
        //samples.push({ valueX: x.date, valueX: x.value });
        let valueX = el.Feature.IsLengthRelated ? x.Length : new Date(x.Date);
        samples.push({ valueX: valueX, valueY: x.Value });
      });
      createSeries(yaxis, el.Material.RawmMaterialName + ' | ' + el.Feature.FeatureName + ' | ' + el.Feature.Unit, samples, yRenderer);
...
function handleData(data) {
  ...
  if (selectedFeatureIds.length > 1) {
    ...
    groupDataForManyMaterials(data)
...
function initSelect() {
  ...
  $("#select_type").kendoDropDownList({
    dataSource: [
      { id: 0, value: '-' },
      { id: 1, value: 'Length related' },
      { id: 2, value: 'Time related' },
    ],
    dataTextField: "value",
    dataValueField: "id",
    select: onFeatureChange,
  });
...
$("#checkbox_norm").change(function () {
  sendMeasurementRequest(selectedMaterialIds, selectedFeatureIds)
});

function sendMeasurementRequest(materialIds = [], featureIds = []) {
  if (!materialIds.length || !featureIds.length)
    return;

  const dataToSend = {
    rawMaterialIds: materialIds,
    featureIds: featureIds,
    timeNormalization: $("#checkbox_norm").is(':checked'),
  }
  AjaxReqestHelperSilent(Url("MeasurementAnalysis", "GetMeasurements"), dataToSend, handleData);

  //const dataToSend = {
  //  rawMaterialIds: [5213, 5214],
  //  featureIds: [5300424],
  //  timeNormalization: $("#checkbox_norm").is(':checked'),
  //}
  //selectedMaterialIds = [5213, 5214];
  //selectedFeatureIds = [5300424];
...
function getFeatureType() {
  const value = $("#select_type").data("kendoDropDownList").value();

  if (value == "1")
    return { lengthRelated: true }
  if (value == "2")
    return { lengthRelated: false }
...
function onFeatureSelect(e) {
  ...
  const dropdownlist = $("#select_type").data("kendoDropDownList");
  ...
  if (selectedItem.IsLengthRelated != null && selectedItem.IsLengthRelated)
    dropdownlist.value(1);
  else if (selectedItem.IsLengthRelated != null && !selectedItem.IsLengthRelated)
  ...
  $("#" + type + "_" + id + " .el-remove").click((el) => {
    ...
    if (type == 'feature') {
      ...
      if (!selectedFeatureIds.length) 
        $("#select_type").data("kendoDropDownList").enable(true);
    }
