...
function initTrackingModes() {
  ...
  switchSlowProd.click(() => {
    switchSlowProdBtn[0].checked ? stopSlowProductionAction() : startSlowProductionAction();
  })

  switchLaneProd.click(() => {
    switchLaneProdBtn[0].checked ? startLaneCommunicationAction() : stopLaneCommunicationAction();
  })
}

function setLaneCommunication(isLaneStopped) {
  switchLaneProdBtnKendo.check(isLaneStopped);
}

function setSlowProduction(isSlowProduction) {
...
function initRefreshHandler() {
  ...
  try {
    ...
    SignalrConnection.on("L1MaterialPositionRefresh", (refreshData) => {
      ...
      refreshGeneralArea(refreshData);
...
let _generalAreasInOrder = [TrackingAreaKeys.GREY_AREA];
let _materialsInGeneralArea = [];

function refreshGeneralArea(newAreasState) {
  let materialsToGeneralArea = [];
  let materialsExcluded = [];// = newAreasState["3105"].Materials || [];

  for (let i = 0; i < _generalAreasInOrder.length; i++) {
    if (!newAreasState[_generalAreasInOrder[i]]) continue;
    let newMaterialsInArea = newAreasState[_generalAreasInOrder[i]].Materials || [];
    newMaterialsInArea.sort((a, b) => (a.PositionOrder > b.PositionOrder) ? 1 : -1);

    for (let j = 0; j < newMaterialsInArea.length; j++) {
      if (!materialsExcluded.some(material => material.RawMaterialId === newMaterialsInArea[j].RawMaterialId)) {
        materialsToGeneralArea.push(newMaterialsInArea[j]);
      }
    }
  }

  if (!compareArraysOfObjects(_materialsInGeneralArea, materialsToGeneralArea)) {
    _materialsInGeneralArea = materialsToGeneralArea;
    placeMaterialsInGeneralArea(_materialsInGeneralArea);
  }
}

function placeMaterialsInGeneralArea(materials) {
  $areaGeneral.empty();
  for (let i = 0; i < materials.length; i++) {
    materials[i].backgroundColor = GetColorModulo16(materials[i].RawMaterialId);
    materials[i].textColor = GetTextColorModulo16(materials[i].RawMaterialId);

    materials[i].displayName = getNameToDisplay((materials[i].MaterialName || materials[i].RawMaterialId), 26);

    let materialEl = createMaterialElement("220px", materials[i])
    $areaGeneral.append(materialEl);
  }
}
