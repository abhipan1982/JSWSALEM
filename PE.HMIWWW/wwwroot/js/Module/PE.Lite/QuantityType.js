...
var columns = ["ActionTypeDescription", "CreatedTs", "LastUpdateTs"];
...
function onElementSelect(e) {
	...
	$('#ActionTypeDetails').addClass('loading-overlay');
	var grid = e.sender;
	var selectedRow = grid.select();
	var selectedItem = grid.dataItem(selectedRow);
	var dataToSend = {
		ElementId: selectedItem.ActionTypeId
	};
	...
	var url = "/ActionType/ElementDetails";
...
function setElementDetailsPartialView(partialView) {
	$('#ActionTypeDetails').removeClass('loading-overlay');
	$('#ActionTypeDetails').html(partialView);
	setTabStripsStates();
}
...
function AddNew() {
	OpenInPopupWindow({
		controller: "ActionType", method: "AddNewActionTypePopupAsync", width: 600, afterClose: reloadKendoGrid
	});
}

function ModifyIncidentDetails(id) {
	OpenInPopupWindow({
		controller: "ActionType", method: "ModifyActionTypeDetailsPopupAsync", width: 600, data: { id: id },afterClose: reloadKendoGrid
	});
}
...
function OnIncidentTypeIdChange(e) {
	...
	$.ajax({
		url: "/ActionType/GetIncidentTypeDetails",
		data: {
...
function EditActionType(id) {
	OpenInPopupWindow({
		controller: "ActionType", method: "ModifyActionTypePopupAsync", width: 600, data: { id: id }, afterClose: reloadKendoGrid
	});
}
...
function EditRecommendedAction(id) {
	OpenInPopupWindow({
		controller: "ActionType", method: "ModifyRecommendedActionPopupAsync", width: 600, data: { id: id }, afterClose: reloadKendoGrid
	});
}

function AddRecommendedAction() {
	OpenInPopupWindow({
		controller: "MaintenanceIncidentType", method: "AddNewRecommendedActionPopupAsync", width: 600, afterClose: reloadKendoGrid
	});
}

function DeleteActionType(itemId) {
	var functionName = DeleteActionType2Confirm;
	var action = 'DeleteActionType';
	PromptMessage(Translations["MESSAGE_deleteConfirm"], "", () => { return functionName(itemId, action) });
}

function DeleteActionType2Confirm(itemId, action) {

	var url = serverAddress + "/ActionType/" + action;
	var data = { Id: itemId };
